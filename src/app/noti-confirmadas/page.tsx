...
const TableCortes: React.FC = () => {
  ...
  const filteredCortes = cortes.filter(corte => {
    return corte.estado === 'confirmado' && Object.values(corte).some(value => 
  ...
  return (
    <DefaultLayout>
      <div className="mx-auto max-w-7xl">
        <Breadcrumb pageName="Notificaciones Confirmadas" />
      </div>
          ...
          <table className="w-full table-auto">
            ...
            <tbody>
              {filteredCortes.length > 0 ? (
                filteredCortes.map((corte) => (
                  <tr key={corte.id}>
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className="estado-confirmado">{corte.estado}</span>
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-4 text-center">No hay cortes confirmados</td>
                </tr>
              )}
            </tbody>
          </table>
